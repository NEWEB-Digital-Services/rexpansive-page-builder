...
var Rexbuilder_Util_Admin_Editor = (function ($) {
	...
	var _receiveMessage = function (event) {
        ...
        Change_UpdateVideoInline_Modal.openModal(event.data.modelData);
			...
			if (event.data.eventName == 'rexlive:editBlockContentPosition') {
				Block_Content_Positions_Modal.openBlockContentPositionModal(
	...
	var setActiveLayout = function (layout) {
		...
		this.$responsiveToolbar
			...
			.parent()
			.removeClass('layout-container--active');
		this.$responsiveToolbar
			.find('.btn-builder-layout[data-name="' + activeLayoutPage + '"]')
			.addClass('active-layout')
			.parent()
			.addClass('layout-container--active');
	...
	var _updateModelState = function () {
		if (
			...
			'false' === $highlightModelEditing.val()
		) {
	...
	var _updateBkgrColTool = function () {
		if ('' !== hightlightRowInfo.color_bg_section) {
			...
			$highlightRowSetBackgroundColor.parent().addClass('tool-button--picker-preview');
			$highlightRowSetBackgroundColor
			...
			$highlightRowSetBackgroundColor.val('').spectrum('set', '');
	...
	var _updateBkgrOverlayTool = function () {
		if ('' !== hightlightRowInfo.row_overlay_color && 'undefined' !== typeof hightlightRowInfo.row_overlay_color) {
			...
			$highlightRowSetOverlay.parent().addClass('tool-button--picker-preview');
			$highlightRowSetOverlay
				.siblings('.tool-button--color-preview')
			...
			$highlightRowSetOverlay.val('').spectrum('set', '');
	...
	var _updateBkgrVidTool = function () {
		if (
			...
			('' !== hightlightRowInfo.video_bg_url_vimeo_section &&
				'undefined' !== typeof hightlightRowInfo.video_bg_url_vimeo_section) ||
			('' !== hightlightRowInfo.video_mp4_url && 'undefined' !== typeof hightlightRowInfo.video_mp4_url) ||
			('' !== hightlightRowInfo.video_bg_id_section && 'undefined' !== typeof hightlightRowInfo.video_bg_id_section)
		) {
	...
	var _initBackgroundColorPicker = function () {
		$highlightRowSetBackgroundColor.each(function (i, el) {
			...
			$el.spectrum({
				replacerClassName:
					'tool-button tool-button--inline tool-button--empty tool-button--color tool-button--spectrum',
				preferredFormat: 'hex',
				showPalette: false,
				showAlpha: true,
				showInput: true,
			...
			_addSpectrumCustomCloseButton($el);
			_addSpectrumCustomSaveButton($el, resetCb);
		});
	};
	...
	var _initOverlayColorPicker = function () {
		$highlightRowSetOverlay.each(function (i, el) {
			...
			_addSpectrumCustomCloseButton($highlightRowSetOverlay, resetCb);
			_addSpectrumCustomSaveButton($highlightRowSetOverlay, resetCb);
		});
	};
  ...
  */
	...
	var init = function () {
		...
		$highlightSectionId = Rexbuilder_Util_Admin_Editor.$responsiveToolbar.find(
			'input[name=toolbox-insert-area--row-id]'
		);
		$highlightModelId = Rexbuilder_Util_Admin_Editor.$responsiveToolbar.find(
			'input[name=toolbox-insert-area--row-model-id]'
		);
		$highlightModelEditing = Rexbuilder_Util_Admin_Editor.$responsiveToolbar.find(
			'input[name=toolbox-insert-area--row-model-editing]'
		);
		...
		$highlightRowSetLayoutCheckbox = Rexbuilder_Util_Admin_Editor.$responsiveToolbar.find(
			'.edit-row-layout-toolbox-checkbox'
		);
