            ...
            AddContainerContext: function ($element, position) {
                ...
                switch (position) {
                    case "inside":
                        ...
                        if (Rexbuilder_Util_Admin_Editor.$frameBuilder.contents().find("body [data-sh-parent-marker]").length != 0)
                            Rexbuilder_Util_Admin_Editor.$frameBuilder.contents().find("body [data-sh-parent-marker]").first().before($contextMarker);
                        else
                            Rexbuilder_Util_Admin_Editor.$frameBuilder.contents().find("body").append($contextMarker);
                        break;
                    case "sibling":
