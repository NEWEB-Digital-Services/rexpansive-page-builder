var Rexbuilder_RexSlider = (function ($) {
    ...
    var _retrieveSliderData = function () {
        ...
        if ($slides.length > 0) {
            ...
            $slides.each(function (i, el) {
                ...
                var imageID = $this_slide_data.find('input[name=rex-slider--slide-id]').val();
                var imageUrl = "";
                var text = $this_slide_data.find('textarea[name=rex-slider--slide-text]').text();
                var videoUrl = $this_slide_data.find('input[name=rex-slider--slide-video-url]').val();
                var videoType = $this_slide_data.find('input[name=rex-slider--slide-video-type]').val();
                var mp4Url = "";
                var videoAudio = $this_slide_data.find('input[name=rex-slider--slide-video-audio]').val();
                var url = $this_slide_data.find('input[name=rex-slider--slide-url]').val();

                if (typeof imageID !== "undefined") {
                    var bgUrl = $this_slide.find('.rex-slider__slide__image-preview').css("background-image");
                    imageUrl = bgUrl.replace('url(', '').replace(')', '').replace(/\"/gi, "");
                }

                if (videoType == "mp4") {
                    mp4Url = $this_slide_data.find('input[name=rex-slider--slide-video-url]').attr("data-mp4-url");
                }
    ...
    var openSlideLinksEditor = function (state, slide) {
        ...
        Rexlive_Modals_Utils.openModal(rexslider_modal_links_editor.$modal_wrap, true);
    };
    ...
    function SlideVideoHandler($data) {
        ...
        slide_uploader_video_frame.on('select', function () {
            ...
            selection.each(function (attachment) {
                ...
                // save id image info
                slide_uploader_video_frame.state('upload-slide-video').get('$data').val(videoObj.videoID);
                slide_uploader_video_frame.state('upload-slide-video').get('$data').attr("data-video-mp4-url", videoObj.videoUrl);
            });
        });
    ...
    var _linkEvents = function () {
         ...
         */
        Rexlive_Base_Settings.$document.on('click', '.rex-slider__slide-edit', function (e) {
            e.preventDefault();
            var $button = $(this);
            ...
            switch (action) {
                ...
                case 'copy':
                    ...
                    rexslider_modal_properties.$slide_list.sortable('refresh');
                    Rexlive_Modals_Utils.resetModalDimensions(rexslider_modal_properties.$modal);
                    break;
                case 'delete':
                    // remove slide
                    $slide.remove();
            ...
            if (rexslider_modal_links_editor.$modal.hasClass('video-links--visible')) {
                ...
                switch (video_type) {
                    ...
                    case 'mp4':
                        ...
                        videoMp4Url = rexslider_modal_links_editor.$video_mp4.attr("data-video-mp4-url");
                        break;
                    case 'vimeo':
                ...
                if ("" != video_information) {
                    $slide_reference.find('input[name=rex-slider--slide-video-url]').val(video_information).attr("data-mp4-url", videoMp4Url);
        ...
        Rexlive_Base_Settings.$document.on("click", "#edit_slider_title_btn", function (e) {
            var $button = $(e.target);
        ...
        Rexlive_Base_Settings.$document.on("click", "#save_slider_title_btn", function (e) {
            var $button = $(e.target);
            var $selectWrap = $button.parents(".rex-slider__import--wrap").children(".rx__select-wrap");
        ...
        Rexlive_Base_Settings.$document.on("click", "#cancel_slider_title_btn", function (e) {
            var $button = $(e.target);
            var $selectWrap = $button.parents(".rex-slider__import--wrap").children(".rx__select-wrap");
            var $inputField = $selectWrap.children(".title-slider");
    ...
    var init = function () {
        ...
        slide_tmpl = $('#rex-slider__new-slide-tmpl').html().trim();
        ...
        rexslider_modal_links_editor.$video_mp4_add = rexslider_modal_links_editor.$modal.find('#rex-slide-choose-mp4');
