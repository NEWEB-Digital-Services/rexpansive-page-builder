var Background_Block_Color_Modal = (function ($) {
    ...
    var _updateColorModal = function (data) {
        ...
        if (data.color != "") {
            ...
            background_block_color_properties.$color_value.spectrum('set', data.color);
        } else {
    ...
    var _launchSpectrumBackgroundColor = function () {
        ...
        background_block_color_properties.$color_palette_buttons.on('click', function (event) {
            var color = $(event.currentTarget).find('.bg-palette-value').val();
            $(event.currentTarget).addClass('palette-color-active');
            background_block_color_properties.$color_preview_icon.hide();
            background_block_color_properties.$color_palette_buttons.not(event.currentTarget).removeClass('palette-color-active');
        ...
        $('.block-background-color-picker .sp-choose').on('click', function () {
            background_block_color_properties.$color_preview_icon.hide();
            background_block_color_properties.$color_runtime_value.val(background_block_color_properties.$color_value.spectrum('get'));
        });

        background_block_color_properties.$color_preview_icon.on('click', function () {
    ...
    var _init = function ($container) {
        ...
        background_block_color_properties = {
            ...
            $color_palette_buttons: $self.find('#bg-block-color-palette .bg-palette-selector'),
        }
