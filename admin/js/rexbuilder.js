...
;(function( $ ) {
	...
	$(function() {
		$('.rex-modal-draggable').draggable({
			cancel: "input,textarea,button,select,option,.rex-check-icon, .input-field, .rex-slider__slide-edit"
		});
		...
		block_content_padding_properties.$trigger.on('click', function() {
			...
			OpenModal(block_content_padding_properties.$modal_wrap, true);
		});
		...
		block_content_padding_properties.$save_button.on('click', function() {
			...
			CloseModal(block_content_padding_properties.$modal_wrap, true);
		});
		...
		block_content_position_properties.$trigger.on('click', function() {
			...
			$text_editor.addClass('push-down-modal');
			OpenModal(block_content_position_properties.$modal_wrap, true);
		});
		...
		block_content_position_properties.$save_button.on('click', function() {
			...
			$text_editor.removeClass('push-down-modal');
			CloseModal(block_content_position_properties.$modal_wrap, true);
		});
		...
		video_modal_properties.$save_button.on('click', function() {
			...
			if( video_modal_properties.$video_youtube_url.val() != '' ) {
				var html = '<li id="block_' + global_section_reference.sectIndex + '_' + global_section_reference.internalIndex + '" class="video with-border item z-depth-1 hoverable svg-ripple-effect" data-block_type="video" data-block-custom-classes=\'\' data-content-padding=\'\' data-bg_settings=\'' + JSON.stringify( bg_setts ) + '\' data-video-has-audio="1">' + 
							element_actions +
				...
				update_live_visual_size( $('#block_'+ global_section_reference.sectIndex + '_' + global_section_reference.internalIndex) );
			...
			CloseModal(video_modal_properties.$modal_wrap);
		});
		...
		$(document).on('click', '.rex-slider__slide-edit', function(e) {
			...
			switch(action) {
				...
				case 'text':
					...
					openTextEditor(slide_selector, slide_text, true, ['hide-padding-position']);
					break;
				case 'video':
					rexslider_modal_properties.$modal.addClass('push-down-modal');
				...
				case 'copy':
					...
					resetModalDimensions( rexslider_modal_properties.$modal );
					break;
				case 'delete':
					// remove slide
					$slide.remove();
		...
		rexslider_modal_links_editor.$cancel_button.on('click', function() {
			...
			CloseModal(rexslider_modal_links_editor.$modal_wrap, true);
		});
		...
		var openSlideLinksEditor = function(state, slide) {
			...
			OpenModal(rexslider_modal_links_editor.$modal_wrap, true);
		};
		...
		var openSliderEditor = function( id, data, slider_id ) {
			...
			if( id && data && slider_id ) {
				...
				// open the editor
				OpenModal(rexslider_modal_properties.$modal_wrap);
			} else {
				rexslider_modal_properties.$slider_autostart.prop('checked', true);
				rexslider_modal_properties.$slider_prev_next.prop('checked', true);
				rexslider_modal_properties.$slider_dots.prop('checked', true);
				...
				this.gridster_options = {
					...
					resize: {
						...
						stop: function(e, ui, $widget) {
							update_live_visual_size($widget);
						},
		...
		var saveAllData = function() {
			...
			if(typeof ready != "undefined" && ready !== null) {
				...
				for(i=0;i<sectionGrid.length;i++) {
					sectionGrid[i] = createSectionShortcode(sectionGrid[i], sectionBackground[i], sectionDimension[i], sectionLayout[i], sectionIdentifiers[i], sectionOverlayColor[i], sectionBackgroundResponsive[i], sectionSeparatorTop[i], sectionSeparatorRight[i], sectionSeparatorBottom[i], sectionSeparatorLeft[i], sectionActivePhotoswipe[i] );
				}
		...
		var createNewTextBlock = function(type, h, w) {
			...
			template = text_template.replace(/\bdata.imageid\b/g, new_id).replace(/\bdata.elementactionsplaceholder\b/g, element_actions).replace(/\bdata.blocktype\b/g, type);

			global_section_reference.gridRef.add_widget(template, h, w);
			update_live_visual_size( $('#' + new_id) );
			global_section_reference.internalIndex++;
		...
		var openTextEditor = function(id, content, target_only, additional_classes) {
			...
			// if( -1 == searchTextFill ) {
				...
				OpenModal( $text_editor_modal_wrap, target_only, additional_classes );
		...
		var openExpandEditor = function(id, content) {
			...
			OpenModal($expand_editor_modal_wrap);
		};
		 ...
		 */
		var createSectionShortcode = function(data, settings, settsdim, settlayout, settIdentifier, settOverlayColor, settBackResp, settSepTop, settSepRight, settSepBottom, settSepLeft, settActivePhotoswipe) {
			var shortcode = '',
				section_settings,
				dimension;
		...
		$(document).on('click', '.builder-copy-row', function(e) {
			...
			$section_copy.find('.item').each(function(i,el) {
				...
				var new_id = this_id.replace( /_(\d+)_/g, '_' + counter + '_' );
		...
		$('#rexeditor-close').on('click', function(e) {
			e.preventDefault();
			$('#editor-save').val('');
			CloseModal($text_editor_modal_wrap, false, ['hide-padding-position']);
			tinyMCE.triggerSave();
		});
		...
		$('#rexeditor-expand-close').on('click', function(e) {
			...
			CloseModal($expand_editor_modal_wrap);
			tinyMCE.triggerSave();
		});
		...
		$(document).on('click', '.background_handler', function(e) {
			...
			OpenModal(background_modal_properties.$modal_wrap);
		...
		$('#background_set_cancel').on('click', function() {
			...
			CloseModal(background_modal_properties.$modal_wrap);
		});
			...
			if( section_id ) { // Edit the background of a section
				...
				switch(type) {
					...
					case 'color':
						...
						if(set.color != 'rgba(255, 255, 255, 0)' && set.color != 'rgba(255,255,255,0)' && set.color != 'transparent') {
							$block_to_save.addClass('no-border').removeClass('with-border');
						} else {
							$block_to_save.addClass('with-border').removeClass('no-border');
						}
			...
			image_uploader_frame.on('select', function() {
				...
				selection.each(function(attachment) {
					...
					if ( 'image' === obj_attachment.type ) {
						...
						var bg_img_type;
						switch(global_section_reference.sectionRef.attr('data-layout')) {
							case 'masonry':
								bg_img_type = 'natural';
								break;
							case 'full':
						...
						var photoswipe;
						switch(global_section_reference.sectionRef.attr('data-section-active-photoswipe')) {
							case '1':
								photoswipe = "true";
								break;
							case '0':
					...
					update_live_visual_size( $('#' + new_block_id ) );
					global_section_reference.internalIndex++;
					launchTooltips();
				});
			...
			video_uploader_frame.on('select', function() {
				...
				selection.each(function(attachment) {
					...
					if ( 'video' === obj_attachment.type ) {
						...
						html = '<li id="block_' + global_section_reference.sectIndex + '_' + global_section_reference.internalIndex + '" class="video with-border item z-depth-1 hoverable svg-ripple-effect" data-block_type="video" data-block-custom-classes=\'\' data-content-padding=\'\' data-bg_settings=\'' + JSON.stringify( bg_setts ) + '\' data-video-has-audio="1">' + 
									element_actions +
									'<div class="element-data">' +
										'<textarea class="data-text-content" display="none"></textarea>' +
									'</div>' +
