 ...
 */
if ( ! class_exists( 'Rexpansive_Classic_Import_Xml_Content' ) ) {
	class Rexpansive_Classic_Import_Xml_Content {
		...
		public function import_post( $item ) {
			...
			if( false === get_post_status( $args['import_id'] ) ) {
				...
				if( $args['post_type'] == 'attachment' && $args['post_status'] != 'trash') {
					/*if( $meta_input['_wp_attached_file'] ) {
						$x = $this->get_year_month_media_info( $meta_input['_wp_attached_file'] );
					}
					var_dump($x);*/
	...
	//			'cat_ID'				=>	(int) $cat_info->term_id,
			...
			if( !is_wp_error( $cat_temp_ID ) ) {

				// global $wpdb;

				// $wpdb->query(
				// 	$wpdb->prepare( "UPDATE $wpdb->terms SET term_id = %d WHERE term_id = %d", $cat_original_id, $cat_temp_ID )
				// );

				// $wpdb->query(
				// 	$wpdb->prepare( "UPDATE $wpdb->term_taxonomy SET term_taxonomy_id = %d, term_id = %d WHERE term_id = %d", $cat_original_id, $cat_original_id, $cat_temp_ID )
				// );
			}
		}
		...
		public function import_term( $term ) {
			...
			if( !is_wp_error( $term_result ) ) {
				...
				// $wpdb->query(
				// 	$wpdb->prepare( "UPDATE $wpdb->terms SET term_id = %d WHERE term_id = %d", $term_original_id, $temp_term_id )
				// );

				// $wpdb->query(
				// 	$wpdb->prepare( "UPDATE $wpdb->term_taxonomy SET term_taxonomy_id = %d, term_id = %d WHERE term_id = %d", $term_original_id, $term_original_id, $temp_term_id )
				// );
			}
		}
