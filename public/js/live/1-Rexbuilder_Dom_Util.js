var Rexbuilder_Dom_Util = (function ($) {
	...
	var _updateSlider = function (data) {
		...
		var $sliderToDestroy = $textWrap.children(
			'.rex-slider-wrap:not([data-rex-slider-number="' + data.sliderNumberToActive + '"])'
		);

		$sliderToDestroy.each(function (i, slider) {
			var $slider = $(slider);
	...
	var _updateRow = function ($section, $sectionData, $galleryElement, rowSettings) {
		...
		var gutter = row_distances.gutter,
			...
			fullHeight =
				typeof rowSettings.full_height === 'undefined' || layout == 'masonry' ? false : rowSettings.full_height,
	...
	var _removeYoutubeVideo = function ($target, removeFromDom) {
		...
		if ($ytpWrapper.length != 0) {
			...
			if (removeFromDom) {
				...
				// );
				var youTubeTemplate =
					'<div class="rex-youtube-wrap" data-property="{videoURL:\'' +
					videoID +
					"',containment:'self',startAt:0,mute:" +
					false +
					',autoPlay:true,loop:true,opacity:1,showControls:false, showYTLogo:false}"></div>';
	...
	var _addMp4Video = function ($target, mp4Data, hasAudio) {
		...
		if (($videoWrap.length != 0 && tempSrcUrl != mp4Data.linkMp4) || $videoWrap.length == 0) {
			...
			var mp4Tmpl =
				'<div class="rex-video-wrap intrinsic-ignore" data-rex-video-width="' +
				mp4Data.width +
				'" data-rex-video-height="' +
				mp4Data.height +
				'"><video width="' +
				mp4Data.width +
				'" height="' +
				mp4Data.height +
				'" class="rex-video-container" preload autoplay loop muted><source type="video/mp4" src="' +
				mp4Data.linkMp4 +
				'"></video></div>';
			...
			if ($target.is('section')) {
				...
				$target.children(insert_after).after(mp4Tmpl);
			} else {
				/* === Working on a block === */
	...
	var _addVimeoVideo = function ($target, urlVimeo, hasAudio) {
		...
		if (urlVimeo == '') {
			...
			if (
				($vimeoWrap.length != 0 && $vimeoWrap.children('iframe').attr('src').split('?')[0] != urlVimeo) ||
				$vimeoWrap.length == 0
			) {
				_removeVimeoVideo($target, true);
				tmpl.arg = 'video';
				...
				var vimeoTemplate =
					'<div class="rex-video-vimeo-wrap rex-video-vimeo-wrap--block"><iframe src="' +
					urlVimeo +
					'" width="640" height="360" frameborder="0" allow="autoplay"  webkitallowfullscreen mozallowfullscreen allowfullscreen></iframe></div>';
	...
	var updateRemovingBlock = function ($elem, hasToBeRemoved, galleryEditorInstance) {
		if (hasToBeRemoved) {
	...
	var _fixSectionDomOrder = function (newOrder, domUpdating, sectionMoved) {
		...
		var z,
			tot_allSections = allSections.length;
	...
	var _updateImageFlexPostition = function ($elem, flexPosition) {
		...
		if (flexPosition.x != '' && flexPosition.y != '') {
			...
			$elemData.attr('data-block_flex_img_position', flexPosition.x + ' ' + flexPosition.y);
		}
	};
	...
	var _updateBlockBackgroundColorLive = function (data, color) {
		...
		if (data.modelNumber != '') {
			...
			$target = Rexbuilder_Util.$rexContainer.find(section_selector).find(block_selector);
			...
			$target = Rexbuilder_Util.$rexContainer
				...
				.find('div [data-rexbuilder-block-id="' + data.rexID + '"] .grid-item-content');
	...
	var _updateBlockOverlayColorLive = function (data, color) {
		...
		if (data.modelNumber != '') {
			...
			$target = Rexbuilder_Util.$rexContainer
				.find('section[data-rexlive-section-id="' + data.sectionID + '"]')
				.find('div [data-rexbuilder-block-id="' + data.rexID + '"] .responsive-block-overlay');
	...
	var _updateSectionVisibility = function ($section, show, layoutsOrder, defaultStateSections) {
		if (show) {
			...
			if (Rexbuilder_Util.activeLayout == 'default') {
				...
				Rexbuilder_Util.removeSectionID($section.attr('data-rexlive-section-id'));
			}
	...
	var _updateSectionBecameModel = function (data) {
		...
		if (data.isModel) {
			...
			if ($section.find('.grid-stack-row').parent().children('.rexpansive-block-grid').length == 0) {
				$section.find('.grid-stack-row').parent().prepend('<div class="rexpansive-block-grid"></div>');
			}
			if ($section.find('.rexpansive-block-section-toolbox').length == 0) {
				$section.find('.section-toolBox').parent().prepend('<div class="rexpansive-block-section-toolbox"></div>');
			}
		} else {
	...
	var _updateRowDistancesData = function ($gallery, newDistances) {
		var $sectionData = $gallery.parents('.rexpansive_section').children('.section-data');
	...
	function _updateBulkSection(targetInfo, changedData, defaultData) {
		...
		if (
			...
			Rexbuilder_Util.globalViewport.width < 768 &&
			-1 !== defaultProps.custom_classes.indexOf('rex-block-grid')
		) {
	...
	var _performAction = function (action, flag) {
		...
		switch (action.actionName) {
			...
			case 'updateBlockVisibility':
				if (galleryEditorInstance !== undefined) {
					updateRemovingBlock(dataToUse.targetElement, dataToUse.hasToBeRemoved, dataToUse.galleryInstance);
				}
				break;
			case 'updateSlider':
			...
			case 'updateBlockImageBG':
				...
				if (galleryEditorInstance !== undefined) {
					if (galleryEditorInstance.settings.galleryLayout == 'masonry') {
						galleryEditorInstance.updateElementHeight(dataToUse.$itemContent.parents('.grid-stack-item').get(0));
					}
				}
